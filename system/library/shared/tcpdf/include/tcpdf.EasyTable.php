...
class TCPDF_EasyTable extends TCPDF
{
		...
		for ($TR = 0; $TR < $tableRowCount; $TR ++){ 			
			...
			switch ((int)$this->_CellFillStyle){
				...
				case 2: // fill alternate cells:					
					if($this->_FillCell == 0) $this->_FillCell = 1;
			...
			if($rowHeight > $PageExtentY){				
				throw new Exception("Row height violation detected. The cell height is too large to fit on a single page, single cells cannot span more than 1 page. Consider splitting the text into multiple cells, or try reducing the font size", 100001);				
			} elseif(($this->y + $rowHeight) > $PageBottomLimit){
	...
	private function _ProcessMultiCellText($cellText, $cellWidth, $cellHeight, $cellIndex)
	{
		...
		$this->MultiCell($cellWidth, $cellHeight, $cellText, 1, $textAlign, $this->_FillCell, 0, '', '', true, 0, true);
	 ...
	 */
	private function _GetRowHeight($tableRow, $rowIndex = 0)
	{
		$maxRowHeight = 0;		
		...
		} else { // for all other cells we calc the height with routine below				
	...
	{		
		...
		$cellWidth = 25;		
		if($this->_CellWidths && isset($this->_CellWidths[$cellIndex])){ // custom column widths have been defined				
			if($this->_CellWidths[$cellIndex] == 'auto'){ // calculate width of auto columns				
				...
				if($this->_HCellSpace){
					$unallocatedWidth -= ($this->_HCellSpace * (sizeof($this->_TableHeaders) - 1));
				}				
			...
			} elseif($this->_CellWidths[$cellIndex] > 0){					
				$cellWidth = (float)$this->_CellWidths[$cellIndex];
			}
		} else {			
			if($this->_TableHeaders){				
				$columnCnt = sizeof($this->_TableHeaders); // we will attempt to automatically calculate the cells widths, cells will be equally spaced across page width...
			} else {				
