  	...
  	public function getProducts() {
		if (!$this->data) {
			...
			foreach ($this->session->data['cart'] as $key => $quantity) {
				$product = explode(':', $key);
				array_push($zproarray,$product[0]);
				...
				//$cartIDS = $newcartproducts; 
					...
					if (is_array($this->session->data['upsell_array'])) {
						...
						if(in_array($key1, $cartIDS))
						{
							...
							if($value1['upsell_productid']==$product_query->row['product_id'])
							{ 

								
								if(in_array($product_query->row['product_id'], $cartIDS)){


//echo "sessionid1".$this->session->data['upsell_array'][$key1]['upsell_status']; 
									$productz=$product_query->row['product_id'];
								...
								//echo "sessionid".$this->session->data['upsell_array'][$key1]['upsell_status']; 
									$counts = array_count_values($zproarray);
   
		
								/*if($newcartproducts[$productz]>1){
									
									$priceval=$value1['upsell_productprice'];
										
										if($value1['upsell_productspecial'])
										{
											
											$priceval=$value1['upsell_productspecial']; 
										} 
										

									$totalpriceval= (($price + $option_price) * ($quantity - 1))+$priceval;
									...
									{ */	 
										...
										$price=$value1['upsell_productprice'];
										
										if($value1['upsell_productspecial'])
										{
											
											$price=$value1['upsell_productspecial']; 
										} 
										
										$totalpriceval= ($price + $option_price) * $quantity;

										//if(isset($this->session->data['upsell_array'][$key1]['upsell_status'])) unset($this->session->data['upsell_array'][$key1]['upsell_status']);
				...
				else{ $totalpriceval= ($price + $option_price) * $quantity; }
		...
		//***upsell product details end here
					...
					$this->data[$key] = array(
						...
						'total'           => $totalpriceval,//($price + $option_price) * $quantity,
